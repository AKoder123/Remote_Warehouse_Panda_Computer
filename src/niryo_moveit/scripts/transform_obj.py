...
class TransformObjectPos():
    ...
    def __init__(self, obj_arr):
        self.visualization = rospy.get_param('~visualization_name')
    ...
    def img_callback(self, msg_img):
        ...
        for obj in self.obj_arr:
            ...
            if self.visualization.lower() == "camera":
                alpha = 0.3
...
if __name__ == '__main__':
    ...
    obj_A = Object(color=(0,0,255),position=Vector3(0.47, 0, 0.165), available=True, index=0)
    obj_B = Object(color=(0, 144, 0), position=Vector3(0.37, -0.33, 0.165), available=True, index=1)
    obj_C = Object(color=(255, 128, 0), position=Vector3(0.42, 0.42, 0.165), available=True, index=2)
    obj_D = Object(color=(255, 178, 129), position=Vector3(0.47, 0.21, 0.165), available=True, index=3)
    # objects_array = [obj_A, obj_B, obj_C, obj_D]
    objects_array = [obj_A]
    '''
    error = rospy.get_param('~error_selected')

    objects_array = []

    if error == "1a":
        A = Object(color=(0,0,255),position=Vector3(0.55, 0.25, 0.165), available=True, index=0)
        B = Object(color=(0, 144, 0), position=Vector3(0.48, -0.015, 0.165), available=True, index=1)
        C = Object(color=(255, 128, 0), position=Vector3(0.4, -0.15, 0.165), available=True, index=2)
        D = Object(color=(255, 178, 129), position=Vector3(0.42, 0.42, 0.165), available=True, index=2)
        E = Object(color=(255, 129, 251), position=Vector3(0.7, 0.28, 0.165), available=True, index=2)
        F = Object(color=(3, 44, 128), position=Vector3(0.7, 0, 0.165), available=True, index=2)
        objects_array = [A, B, C, D, E, F]
    elif error == "1b":
        A = Object(color=(0,0,255),position=Vector3(0.47, 0.21, 0.165), available=True, index=0)
        B = Object(color=(0, 144, 0), position=Vector3(0.6, 0.355, 0.165), available=True, index=1)
        C = Object(color=(255, 128, 0), position=Vector3(0.4, -0.15, 0.165), available=True, index=2)
        D = Object(color=(255, 178, 129), position=Vector3(0.65, 0.18, 0.165), available=True, index=2)
        E = Object(color=(255, 129, 251), position=Vector3(0.6, 0, 0.165), available=True, index=2)
        F = Object(color=(3, 44, 128), position=Vector3(0.49, -0.1, 0.165), available=True, index=2)
        objects_array = [A, B, C, D, E, F]
    elif error == "2a":
        obj_A = Object(color=(0,0,255),position=Vector3(0.47, 0, 0.165), available=True, index=0)
        objects_array = [obj_A]
    elif error == "2b":
        obj_A = Object(color=(0, 144, 0), position=Vector3(0.37, -0.33, 0.165), available=True, index=0)
        objects_array = [obj_A]
